...
class PrinterSetup(QWidget):
    ...
    def populate_printer_options(self):
        ...
        try:
            ...
            # Fallback: always include Penrose variants so users can select them
            # even if the PRINTER_*.cfg files are not found by discovery yet.
            # Use case-insensitive check to avoid duplicates.
            fallback_printers = ["PENROSE_SINGLE", "PENROSE_DUAL"]
            for fb in fallback_printers:
                if not any(fb.lower() == p.lower() for p in available_printers):
                    available_printers.append(fb)
