...
class ThreadConnectionCheck(QtCore.QThread):
    ...
    def run(self):
        ...
        self.shutdown_flag = False
        uptime = 0
        OCTOPRINT_TIMEOUT = 60  # seconds - increased for OctoPrint 1.11.x startup migration
        ...
        while True:
            try:
                ...
                if not self.virtual:
                    ...
                    KLIPPER_WAIT_TIMEOUT = 30  # seconds
                    klipper_wait = 0
...
class MainController(QtCore.QObject):
    ...
    def __init__(self):
        ...
        # When the printer configuration (IS_DUAL_NOZZLE etc.) is reloaded,
        # re-apply nozzle-specific UI changes so tool/heater widgets update.
        try:
            self.printer_model.printer_config_updated.connect(lambda cfg: self.main_window._apply_nozzle_configuration())
        except Exception:
            self.logger.debug("Failed to connect printer_config_updated signal to UI updater")
